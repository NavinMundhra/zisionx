...
def index_faces_in_s3_path(bucket_name, phone_number, max_workers=10):
    """
    ...
    try:
        ...
        print(f"Error processing bucket {bucket_name} for path {phone_number}: {e}")


if __name__ == "__main__":
    import sys

    # Check if phone_number is provided as a command-line argument
    if len(sys.argv) < 2:
        print("Usage: python index_faces_in_s3.py <phone_number>")
        sys.exit(1)

    PHONE_NUMBER = sys.argv[1]  # Get phone_number from command-line argument

    print(f"Starting face indexing for phone number: {PHONE_NUMBER}")
    empty_collection_and_dynamodb(COLLECTION_ID, DYNAMODB_TABLE)
    index_faces_in_s3_path(S3_BUCKET, PHONE_NUMBER)